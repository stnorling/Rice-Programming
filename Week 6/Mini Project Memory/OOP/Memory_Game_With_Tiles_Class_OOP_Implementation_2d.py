...
def new_game():
    ...
    my_tiles = [Tile(tile_numbers[i], False, [TILE_WIDTH * i, TILE_HEIGHT]) for i in range(DISTINCT_TILES)]
    my_tiles_bottom =  [Tile(tile_numbers[i], False, [TILE_WIDTH * (i - DISTINCT_TILES), TILE_HEIGHT * 2]) for i in range(DISTINCT_TILES, DISTINCT_TILES * 2)]
    my_tiles.extend(my_tiles_bottom)
