...
def evaluate(ops, eqn):
    ...
    (Computes the value of the equation here)
    """
    val = eqn[0]
    idx = 1
    while idx < len(eqn):
...
def string_equation(eqn):
    ...
    Note: 	Will result in error if number of dice (outcomes) < 3, and 
            will not show full equation and result if number of dice > 3
    """
