...
# powerUp image
powerUpInfo = ImageInfo([10, 10], [20, 20], 10, 200)
powerUpImage = simplegui.load_image("http://commondatastorage.googleapis.com/codeskulptor-assets/lathrop/shot3.png")
...
powerUpSound = simplegui.load_sound("https://www.myinstants.com/media/sounds/ice-barrage1.mp3")
powerUpSound.set_volume(.7)
...
class Ship:
    ...
    def shoot(self, angle):
        ...
        self.forward_vector = angle_to_vector(self.angle + angle)
        if powerUp:
            missile_im = missile_image2
        else:
            missile_im = missile_image
        a_missile = Sprite([self.pos[0] + self.forward_vector[0] * self.radius, self.pos[1] + self.forward_vector[1] * 45], 
                          ...
                          0, 10, missile_im, missile_info, missile_sound)
...
def draw(canvas):
    global time, in_play, lives, score, rock_group, timer, powerUpGroup, powerUp
    ...
    process_sprite_group(powerUpGroup, canvas)
    ...
    if group_collide(powerUpGroup, my_ship):
        powerUp = True;
        powerUpSound.play()
...
def group_collide(group, other_object):
    ...
    for sprite in group.copy():
        if sprite.collision(other_object):
            ...
            if other_object == my_ship and group != powerUpGroup:
...
# timer handle event for powerup
def timer():
    global powerUp, powerUpTimer
    rock_spawner()
    ...
    if powerUp == True:
        if powerUpTimer<8:
            ...
            powerUpTimer=0
            powerUp = False     
    elif random.randint(0,10)>9 and len(powerUpGroup) < 1:
        PowerUpSpawner()
        
# handler that spawns a rock    
def rock_spawner():
    global rock_group
...
# handler that spawns power ups
def PowerUpSpawner():
    global powerUpGroup
    
    vel = 0.5
    rand_pos = [random.randint(20, WIDTH - 20), random.randint(20, HEIGHT - 20)]
    rand_vel = [random.random() * random.choice([vel, -vel]), 
                random.random() * random.choice([vel, -vel])]
    rand_angular_vel = random.random() * 0.1 * random.choice([1, -1])
    
    a_powerUp = Sprite(rand_pos, rand_vel, random.random(), rand_angular_vel, powerUpImage, powerUpInfo)
    
    if dist(a_powerUp.get_pos(), my_ship.get_pos()) > (a_powerUp.get_radius() + my_ship.get_radius()) * 2:
        powerUpGroup.add(a_powerUp)
