...
class Person:
    ...
    def __str__(self):
        return "The person's name is " + self.full_name() + ". Their birth year is " + \
        str(self.birth_year)
...
def average_age2(person_list, current_year):
    ta = 0.0
    for p in person_list:
        ta += p.age(current_year)
    return ta/len(person_list)


def average_age(person_list, current_year):
    return sum([p.age(current_year) for p in person_list]) / float(len(person_list))
